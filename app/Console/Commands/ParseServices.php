...
class ParseServices extends Command
{
    ...
    private function getRoot()
    {
        ...
        return collect($services)->map(function($service) {
            ...
            $this->info('** Parsing ' . $service->hostname);

            return collect($service->routes)->map(function($route) {
                return [
                    'id' => $route->id,
                    'method' => $route->type,
                    'path' => $route->path,
...
//                return $this->injectSettings($this->parseSwaggerResources($data['apis']), $url, $docRoot, $service->hostname);

            if (isset($data['swagger']) && preg_match('/^2\./', $data['swagger']) && isset($data['paths']))
                return $this->injectSettings($this->parseSwaggerTwo($data), $url, $docRoot, $service->hostname);
        ...
        return collect($paths)->reduce(function($carry, $route) {
            ...
            foreach ($route['operations'] as $realOperation) {
                $carry[] = [
                    ...
                    'public' => count($realOperation['security']) > 0 ? false : true,
                    'actions' => [[
                        'method' => $realOperation['method'],
    ...
    public function handle()
    {
        $output = $this->getActions(
            collect($this->getPaths($this->getRoot()))
        );
